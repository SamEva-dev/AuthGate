...
<body>
    <div class="container">
        ...
        <div class="content">
            ...
            <p>En acceptant cette invitation, vous pourrez accéder à la plateforme et collaborer avec votre équipe.</p>
...
""";
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            _logger.LogInformation("Email sending disabled. Would queue to {Email}: {Subject}", toEmail, subject);
            return Task.CompletedTask;
        }

        return _emailing.QueueHtmlAsync(
            toEmail: toEmail,
            subject: subject,
    ...
    private static EmailUseCaseTags InferTags(string subject)
    {
        var s = (subject ?? string.Empty).Trim();

        if (s.Contains("Invitation", StringComparison.OrdinalIgnoreCase))
            return EmailUseCaseTags.AccessInviteUser;

        if (s.Contains("Bienvenue", StringComparison.OrdinalIgnoreCase))
            return EmailUseCaseTags.NotificationWelcome;

        if (s.Contains("Réinitialisation", StringComparison.OrdinalIgnoreCase))
            return EmailUseCaseTags.AuthResetPassword;

        if (s.Contains("Vérifiez", StringComparison.OrdinalIgnoreCase) || s.Contains("Vérifier", StringComparison.OrdinalIgnoreCase))
            return EmailUseCaseTags.AuthConfirmEmail;

        return EmailUseCaseTags.NotificationSystem;
    }
}
